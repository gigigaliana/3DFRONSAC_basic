function [ReconObject] = Ainv_timesb_psf(A_vars,FT_data)
...
%image
%WARNING: this function assumes that recon size in y and z = Nyq_x and Nyq_z
...
[Nx Ny Nz ncoils]= size(B1_2DxCoil);
[Ns NumComponents]=size(traj_mat_1y1z);
ReconObject     =zeros(Nx,Ny,Nz);

FT_data=reshape(FT_data,[Ns Ny/Ry Nz/Rz ncoils]); 
...
elseif A_vars.CentralSliceOnlyFlag==1
    ...
    for  k=z_start:z_end
            ...
            for j=1:Ny/Ry
              ...
              ReconObject(:,ky_set,kz_set)=reshape( transpose(A)*reshape(FT_data(:,j,k,:),[Ns*ncoils 1]), [Nx Ry Rz]);
