...
elseif A_vars.CentralSliceOnlyFlag==1
    ...
    disp('Reconstructing a single z-slice (or slice set)');
end;
...
for  k=z_start:z_end
    ...
    for j=1:Ny/Ry
        ...
        %                                             ReconObject(:,ky_set,kz_set)=reshape(pinv(A)*reshape(FT_data_flip(:,j,k,:),[Ns*ncoils 1]), [Nx Ry Rz]);
        ReconObject(:,ky_set,kz_set)=reshape(lsqminnorm(A,reshape(FT_data(:,j,k,:),[Ns*ncoils 1])), [Nx Ry Rz]);
